...
async function compileStyles() {
  ...
  for (const file of styleFiles) {
    ...
    const outputFile = path.join(outputDirs.es, file.replace(/\.scss$/, '.css').replace(/\.sass$/, '.css'));
...
function getEntries() {
  ...
  const components = glob
    ...
    .filter((dir) => {
      return !['node_modules', 'es', 'lib', 'dist', 'style', 'utils', 'scripts'].includes(dir);
    });
  ...
  const plugins = [
    ...
    replace({
      ...
      values: {
        ...
        PKG_VERSION: JSON.stringify(require(path.join(rootDir, 'package.json')).version),
      },
    }),
  ];
...
async function buildES() {
  ...
  const bundle = await rollup({
    input: entries,
    external: (id) => {
      const externals = ['vue', 'tdesign-vue', 'tdesign-icons-vue', 'lodash-es', 'clipboard'];
