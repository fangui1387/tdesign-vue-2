...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const copyAnswer = () => {
      ...
      copyBtn.on('success', () => {
        messagePluginInstance.showSuccess(globalConfig.value.copyTextSuccess);
      });
      copyBtn.on('error', () => {
        messagePluginInstance.showError(globalConfig.value.copyTextFail);
      });
    };
    ...
    const renderContent = () => {
      ...
      const { copyTipText, likeTipText, dislikeTipText, refreshTipText, shareTipText } = globalConfig.value;
      ...
      return (
        <div class={`${COMPONENT_NAME.value}__actions`}>
          {actionButtons.map((btnKey) => buttonComponents[btnKey])}
        </div>
      );
    };
