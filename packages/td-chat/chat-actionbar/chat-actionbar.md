...
actionBar | Array | ["replay", "copy", "good", "bad"] | 操作按钮配置项，可配置操作按钮选项和顺序。TS 类型：`Array<'replay' \| 'copy' \| 'good' \| 'bad' \| 'share'>` | N
operationBtn | Array | ["replay", "copy", "good", "bad"] | 操作按钮配置项，可配置操作按钮选项和顺序（待废弃，请尽快使用actionBar）。TS 类型：`Array<'replay' \| 'copy' \| 'good' \| 'bad' \| 'share'>` | N
...
operation | `(value: string, context: { e: MouseEvent })` | 点击点赞，点踩，复制，重新生成按钮时触发（待废弃，请尽快使用actions）
