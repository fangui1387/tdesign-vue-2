...
export default {
  ...
  methods: {
    handleAction(type, context) {
      console.log('Action clicked:', type, context);
    },
    handleDynamicAction(type) {
      if (type === 'good') {
        this.dynamicComment = this.dynamicComment === 'good' ? '' : 'good';
      } else if (type === 'bad') {
        this.dynamicComment = this.dynamicComment === 'bad' ? '' : 'bad';
      }
      console.log('Dynamic action clicked:', type, 'Current comment:', this.dynamicComment);
    }
  }
};
</script>
